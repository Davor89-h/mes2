...
function saveToFile() {
  ...
  try {
    ...
    console.error('⚠️  Greška pri spremanju baze:', err.message)
  }
}
...
async function init() {
  ...
  if (fs.existsSync(DB_PATH)) {
    try {
      ...
      // Provjeri je li datoteka validna SQLite baza (mora počinjati s "SQLite format 3")
      if (buf.length < 16 || buf.toString('utf8', 0, 6) !== 'SQLite') {
        throw new Error('Datoteka nije valjana SQLite baza')
      }
      ...
      if (result !== 'ok') {
        throw new Error('Provjera integriteta baze nije prošla: ' + result)
      }
    } catch (err) {
      console.warn('⚠️  Baza podataka je korumpirana, kreiranje nove:', err.message)
      // Backup stare baze
      const backupPath = DB_PATH + '.corrupted.' + Date.now()
      try { fs.renameSync(DB_PATH, backupPath) } catch(e) {}
